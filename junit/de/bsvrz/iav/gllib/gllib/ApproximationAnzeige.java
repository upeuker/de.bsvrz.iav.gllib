...
public final class ApproximationAnzeige extends JFrame implements ItemListener {
		...
		public void paint(final Graphics g) {
			...
			if (showPolyline.isSelected()) {
				...
				for (final Stuetzstelle<Double> s : polyline) {
					if (von != null) {
						g2.drawLine((int) von.getZeitstempel(), von.getWert()
								.intValue(), (int) s.getZeitstempel(), s
								.getWert().intValue());
					}
					von = s;
				}
			...
			for (final Stuetzstelle<Double> s : stuetzstellen) {
				g2.drawRect((int) s.getZeitstempel() - 5, s.getWert()
						.intValue() - 5, 10, 10);
			}
		}
	...
	private ApproximationAnzeige() {
		...
		bSplineOrdnung = new JSpinner(new SpinnerNumberModel(5, 1,
				stuetzstellen.size(), 1));
		bSplineOrdnung.addChangeListener(new ChangeListener() {

			public void stateChanged(final ChangeEvent e) {
				appBSpline.setOrdnung(((SpinnerNumberModel) bSplineOrdnung
						.getModel()).getNumber().byteValue());
				appBSpline.initialisiere();
				bspline.clear();
