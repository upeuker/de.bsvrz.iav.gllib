...
/**
 * Repr&auml;sentiert eine allgemeine Ganglinie, bestehend aus einer sortierten Menge
 * von St&uuml;tzstellen und der Angabe eines Interpolationsverfahren. Wird kein
 * Approximationsverfahren festgelegt, wird ein {@link BSpline B-Spline} mit
 * Standardordnung angenommen. 
 * 
...
public class Ganglinie extends TreeSet<Stuetzstelle> {
	 ...
	 * 
	 * @param approximation Objekt einer Approximation
	 */
	...
	public void setApproximation(Class<Approximation> approximation) {
		...
		try {
			c = approximation.getDeclaredConstructor(Ganglinie.class);
	...
	public Intervall getIntervall() {
		try {
			return new Intervall(first().getZeitstempel(),
					last().getZeitstempel());
	 ...
	 * 
	 * @param zeitstempel zu pr&uuml;fender Zeitstempel
	 * @return <code>true</code>, wenn <code>zeitstempel</code> zwischen den
	 * Zeitstempeln der ersten und letzten St&uuml;tzstelle liegt, sonst
	 * <code>false</code>
	 */
	public boolean contains(long zeitstempel) {
		...
		return (first().getZeitstempel() <= zeitstempel
		&& zeitstempel <= last().getZeitstempel());
	}
	
	/**
	 * Addiert eine Ganglinie zu der dieser
	 *
	 * @param ganglinie Summand
	 */
	...
	public Float basisAbstand(Ganglinie ganglinie) {
		...
		assert ( 0<= abstand && abstand <= 1) :
			Messages.get(GlLibMessages.Common_BadPercentage, abstand);
	 ...
	 * 
	 * @param ganglinien Ganglinien
	 */
	public void konkatenation(Collection<Ganglinie> ganglinien) {
		// TODO Automatisch erstellter Methoden-Stub
	}
	...
	/**
	 * F&uuml;hrt ein Pattern-Matching dieser Ganglinie mit einer Liste von Ganglinie
	 * durch
	 * 
	 * @param ganglinien Ganglinie
	 * @param offset Offset, um den die Ganglinie zeitlich nach vorn und hinten
	 * verschoben wird
	 * @param maxAbstand Maximaler Abstand den eine Ganglinie beim
	 * Pattern-Matching haben darf, wird dieser Wert &uuml;berschritten wird
	 * <code>-1</code> zur&uuml;ckgegeben.
	 * @return Index der am besten passenden Ganglinie aus der Eingangsliste
	 * oder <code>-1</code>
	 */
	public int patternMatching(Ganglinie ganglinien[], long offset,
			int maxAbstand) {
		// TODO Automatisch erstellter Methoden-Stub
		return -1;
	}
	 ...
	 * 
	 * @param offset Offset, negativ = in Richtung Vergangheit, postiv = in
	 * Richtung Zukunft 
	 */
