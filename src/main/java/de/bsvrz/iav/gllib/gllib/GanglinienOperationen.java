...
public final class GanglinienOperationen {
	...
	public static final class PatternMatchingErgebnis<T extends Ganglinie<?>>
		...
		public String toString() {
			...
			s = getClass() + "["; //$NON-NLS-1$
			s += "index=" + index; //$NON-NLS-1$
			s += ", abstand=" + abstand; //$NON-NLS-1$
			s += ", offset=" + Timestamp.relativeTime(offset); //$NON-NLS-1$
			s += ", ganglinie=" + ganglinie; //$NON-NLS-1$
			s += "]"; //$NON-NLS-1$
	...
	public static String formatierterText(final Ganglinie<?> g) {
		...
		txt = "Intervall: " + g.getIntervall(); //$NON-NLS-1$
		txt += "\nApproximation: " + g.getApproximation(); //$NON-NLS-1$
	...
	public static Ganglinie<Double> addiere(final Ganglinie<Double> g1,
		...
		if ((((g1.getApproximation() != null)
				&& (g2.getApproximation() != null))
				&& !g1.getApproximation().getClass()
						.equals(g2.getApproximation().getClass()))
				|| ((g1.getApproximation() == null)
						^ (g2.getApproximation() == null))) {
			throw new IllegalArgumentException(
					GlLibMsg.IllegalArgumentExceptionUnterschiedlicheApproximation
							.toString());
		}
		...
		for (final long z : zeitstempel) {
			if (g1.isValid(z) && g2.isValid(z)) {
				...
				letzte = p2.getStuetzstellen()
						.get(p2.getStuetzstellen().size() - 1);
				...
				letzte = p1.getStuetzstellen()
						.get(p1.getStuetzstellen().size() - 1);
	...
	public static Ganglinie<Double> normiere(final Ganglinie<Double> g,
		...
		for (long i = g.firstKey() + halberAbstand; i < g
				.lastKey(); i += abstand) {
			...
			if (g.isValid(intervall)) {
				stuetzstellen.put(i,
						p.integral(intervall) / intervall.getLength());
			} else {
	...
	public static PatternMatchingErgebnis<Ganglinie<Double>> patternMatching(
			...
			final List<Ganglinie<Double>> vergleichsListe, final long offsetVor,
	...
	public static Ganglinie<Double> verbinde(final Ganglinie<Double> g1,
		...
		if (g1.getIntervall().intersect(g2.getIntervall())) {
			throw new IllegalArgumentException(
					GlLibMsg.IllegalArgumentExceptionGanglinienUeberschneidenSich
							.toString());
		}
	...
	public static Ganglinie<Double> verschiebeUmHalbesIntervall(
		...
		if (g.size() < 2) {
			throw new IllegalArgumentException(
					GlLibMsg.IllegalArgumentExceptionMin2Stuetzstellen
							.toString());
		}
	...
	private static double fehler(final Ganglinie<Double> g1,
		...
		for (final long z : zeitstempel) {
			if ((g1.getStuetzstelle(z).getWert() != null)
				...
				x = (g1.getStuetzstelle(z).getWert()
						+ g2.getStuetzstelle(z).getWert()) / 2;
				summe += x * x;
			}
		}
		return (fehler * 100)
