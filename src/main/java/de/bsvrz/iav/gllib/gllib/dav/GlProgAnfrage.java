...
public class GlProgAnfrage {
	...
	public Data getDaten(final Data daten) {
		...
		daten.getReferenceValue("Messquerschnitt").setSystemObject( //$NON-NLS-1$
				messQuerschnitt.getSystemObject());
		daten.getTimeValue("ZeitpunktPrognoseBeginn").setMillis( //$NON-NLS-1$
		...
		if (nurLangfristigeAuswahl) {
			daten.getUnscaledValue("NurLangfristigeAuswahl").setText("Ja"); //$NON-NLS-1$ //$NON-NLS-2$
		} else {
			daten.getUnscaledValue("NurLangfristigeAuswahl").setText("Nein"); //$NON-NLS-1$ //$NON-NLS-2$
		}

		if (zyklischePrognose) {
			daten.getUnscaledValue("ZyklischePrognose").setText("Ja"); //$NON-NLS-1$ //$NON-NLS-2$
		} else {
			daten.getUnscaledValue("ZyklischePrognose").setText("Nein"); //$NON-NLS-1$//$NON-NLS-2$
		}

		feld = daten.getArray("EreignisTyp"); //$NON-NLS-1$
		feld.setLength(ereignisTypen.size());
		i = 0;
		for (final EreignisTyp et : ereignisTypen) {
			feld.getItem(i++).asReferenceValue()
					.setSystemObject(et.getSystemObject());
		}
	 ...
	 *            werden soll.
	 * @return {@code true}, wenn der Typ enthalten war und {@code false}, wenn
	 *         er bereits enthalten war.
	 */
	...
	public void setDaten(final Data daten) {
		...
		sendeIntervall = daten.getScaledValue("Aktualisierungsintervall") //$NON-NLS-1$
				.longValue()
				* Constants.MILLIS_PER_SECOND;

		start = daten.getTimeValue("ZeitpunktPrognoseBeginn").getMillis(); //$NON-NLS-1$
		...
		if (daten.getUnscaledValue("ZyklischePrognose").intValue() == 0) { //$NON-NLS-1$
	 ...
	 *            werden sollen.
	 */
	public void setNurLangfristigeAuswahl(
			final boolean nurLangfristigeAuswahl) {
		this.nurLangfristigeAuswahl = nurLangfristigeAuswahl;
	}
	...
	public String toString() {
		...
		s += "messQuerschnitt=" + messQuerschnitt; //$NON-NLS-1$
		s += ", prognoseZeitraum=" + prognoseZeitraum; //$NON-NLS-1$
		s += ", nurLangfristigeAuswahl=" + nurLangfristigeAuswahl; //$NON-NLS-1$
		s += ", ereignisTypen=" + ereignisTypen; //$NON-NLS-1$
		s += ", zyklischePrognose=" + zyklischePrognose; //$NON-NLS-1$
		s += ", pruefIntervall=" + pruefIntervall; //$NON-NLS-1$
		s += ", schwelle=" + schwelle; //$NON-NLS-1$
		s += ", sendeIntervall=" + sendeIntervall; //$NON-NLS-1$
