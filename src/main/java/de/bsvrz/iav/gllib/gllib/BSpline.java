...
public class BSpline extends AbstractApproximation<Double> {
	...
	public void initialisiere() {
		...
		intervall = getStuetzstellen().get(getStuetzstellen().size() - 1)
				.getZeitstempel() - getStuetzstellen().get(0).getZeitstempel();
	...
	public void setOrdnung(final int ordnung) {
		if ((ordnung < 1) || (ordnung > getStuetzstellen().size())) {
			throw new IllegalArgumentException(
					GlLibMsg.IllegalArgumentExceptionBSplineOrdnung.toString());
		}
	...
	public String toString() {
		return "B-Spline mit Ordnung " + ordnung; //$NON-NLS-1$
	}
